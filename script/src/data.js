define(function(require) {
	...
	var database = {
		login: function(username, password, autologin) {
			...
			return this.post('/account_api/user/login', data);
		},

		logout: function() {
			return this.post('/account_api/user/logout');
		},

		add_channel: function(data) {
			return this.post('/admin/channel/add', data);
		},

		update_channel: function(data) {
			return this.post('/admin/channel/add', data);
		},
		...
		networkError: function(XMLHttpRequest, textStatus, errorThrown) {
			...
			seajs.log('网络错误[url:' + this.url + '] ' + errorMessage);
		...
		systemError: function(data, textStatus, jqXHR) {
			...
			if (systemErrors.hasOwnProperty(data.code)) {
				seajs.log('通用错误[url:' + this.url + '] ' + systemErrors[data.code]);
		...
		get: function(url, data) {
			data = data || {};
			// url = 'http://autodmp.cig.com.cn' + url;
			url += '?' + $.param(data);

			var option = {
				url: this.addtimestamp2url(url),
